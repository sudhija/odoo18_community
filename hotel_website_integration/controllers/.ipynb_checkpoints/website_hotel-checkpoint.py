...
class WebsiteIsmHotel(http.Controller):
    ...
    def hotel_list(self, checkin=None, checkout=None, guests=2, category_id=None, **kw):
        ...
        category_id = (kw.get('category_id') or category_id or 0)
        ...
        if category_id:
            domain.append(('room_type', '=', category_id))
            try:
                room_type_field = request.env['hotel.room']._fields['room_type']
                # If your room_type is a selection field, map the persons count
                # Otherwise, fetch from model (e.g., hotel.room.type)
                RoomType = request.env['hotel.room.type'].sudo()
                cat = RoomType.browse(int(category_id))
                if cat and getattr(cat, 'default_persons', False):
                    guests = cat.default_persons
            except Exception as e:
                _logger.warning("Error setting guests from category: %s", e)
        ...
        room_type_field = request.env['hotel.room']._fields['room_type']
        categories = [{'id': val, 'name': label} for val, label in room_type_field.selection]
    ...
    def hotel_booking_checkout(self, room_id, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            try:
                ...
                # Ensure sale order has a valid pricelist before confirming
                try:
                    # prefer order pricelist, then partner default, then product.list0
                    ...
                    if not pricelist:
                        try:
                            pricelist = request.env.ref('product.list0').id
                        except Exception:
                            pl = request.env['product.pricelist'].sudo().search([], limit=1)
                            pricelist = pl.id if pl else False
                    if pricelist:
                        order.sudo().write({'pricelist_id': pricelist})
                    # NOTE: Do NOT auto-confirm the sale order here. Auto-confirming
                    # immediately moves the order out of the website cart state
                    # and causes the standard `/shop/checkout` route to redirect
                    # back to `/shop` (because there is no draft/cart order).
                    # Keep the order in draft so the customer can complete the
                    # checkout on `/shop/checkout`. Booking confirmation should
                    # occur after payment or via a separate reservation flow.
                except Exception as e:
                    _logger.exception('Error while setting pricelist/confirming order %s: %s', order.id, e)
