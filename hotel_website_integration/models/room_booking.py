...
class HotelBooking(models.Model):
    ...
    # room_id is optional when creating a booking that will have lines added later
    room_id = fields.Many2one('hotel.room', string='Room', required=False)
    ...
    def create_reservation_from_sale_order(self, sale_order):
        ...
        for line in sale_order.order_line.filtered(lambda l: l.room_id):
            ...
            if sale_order and sale_order.pricelist_id:
                ...
                if not pricelist_id:
                    raise ValidationError(_("No pricelist found in system — please configure at least one."))
    ...
    def create(self, vals):
        ...
        # If room_id is not provided (booking created as a container for lines),
        # skip overlap checks here. Overlap is enforced on booking lines or via
        # constraints when a room is actually assigned.
        if room_id:
