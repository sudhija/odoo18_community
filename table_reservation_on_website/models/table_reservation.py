...
class TableReservation(models.Model):
    ...
    booked_tables_ids = fields.Many2many(
        ...
        string="Tables", required=True, help="Booked tables")
    ...
    @api.onchange('date', 'starting_at', 'ending_at')
    def _onchange_booked_tables_ids(self):
        """
        Dynamically restrict booked_tables_ids to only show tables that are available
        during the selected date and time range.
        """
        if not self.date or not self.starting_at or not self.ending_at:
            return
    
        try:
            start_time = datetime.strptime(f'{self.date} {self.starting_at}', '%Y-%m-%d %H:%M')
            end_time = datetime.strptime(f'{self.date} {self.ending_at}', '%Y-%m-%d %H:%M')
        except Exception:
            return
        ...
        overlapping_reservations = self.env['table.reservation'].search([
            ('id', '!=', self.id),
            ('date', '=', self.date),
            ('start_time', '<', end_time),
            ('end_time', '>', start_time),
        ...
        unavailable_ids = overlapping_reservations.mapped('booked_tables_ids').ids
    
        return {
            'domain': {
                'booked_tables_ids': [('id', 'not in', unavailable_ids)]
            }
        }
