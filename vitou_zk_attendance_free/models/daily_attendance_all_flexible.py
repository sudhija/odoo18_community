...
class ZkMachineAttendanceFlexible(models.Model):
    """Model to hold data from the biometric device"""
    _name = 'vitouzkf.attendance.all.flexible'
    _description = 'Attendance All Flexible Shift'
    ...
    employee_id = fields.Many2one(comodel_name='hr.employee', string="Employee ID", store=True, domain=[('zk_isfixshift','=',False)], required=True)
    ...
    date_inout = fields.Date(string='Date InOut', default=None, store=True,compute='_update_day')
    ...
    shift3_id = fields.Many2one(related='employee_id.shift3_id', string="Time Shift3", store=True)
    shift4_id = fields.Many2one(related='employee_id.shift4_id', string="Time Shift4", store=True)
    ...
    time_in_base2 = fields.Float(related='employee_id.shift2_id.shift_in',widget='float_time', string="Time In B2" ,help="Time Im Base2  Must be 24 hours format", store=True)
    ...
    time_in_base3 = fields.Float(related='employee_id.shift3_id.shift_in',widget='float_time', string="Time In B3" ,help="Time Im Base3  Must be 24 hours format", store=True)
    time_out_base3 = fields.Float(related='employee_id.shift3_id.shift_out', widget='float_time', string="Time Out B3" ,help="Time Out Base3 Must be 24 hours format", store=True)

    time_in_base4 = fields.Float(related='employee_id.shift4_id.shift_in',widget='float_time', string="Time In B4" ,help="Time Im Base4  Must be 24 hours format", store=True)
    time_out_base4 = fields.Float(related='employee_id.shift4_id.shift_out', widget='float_time', string="Time Out B4" ,help="Time Out Base4 Must be 24 hours format", store=True)
    ...
    def run_chunker(self, data, step):
        func = self.env[func_mod]
        for pair in func.chunker(data, step):
            print(pair)
